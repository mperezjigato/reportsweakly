...
The following script is shown for future reference. It is suggested in a PRACE publication on GPAW [^4], in order to test the initialisation part using the import Python command:
```python
...
```
The full set of calculations we carried out above under MPI will be repeated, this time under hybrid parallelisation. Results are shown below:
